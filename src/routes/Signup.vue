...
export default {
  ...
  methods: {
    async submit() {
      ...
      if ("errors" in signup) return (this.errors = signup.errors);
      if ("error" in signup) return (this.errors = { email: [signup.error] });
      if ("error" in signup)
...
<template>
  <div class="main-body">
    ...
    <router-link class="redirect-create-acc" :to="'/login'"
      >Oups! J'ai déjà un compte</router-link
    >
  </div>
</template>
